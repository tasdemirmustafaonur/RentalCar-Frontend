...
export class CarAddComponent implements OnInit {
  ...
  add() {
    if (!this.carAddForm.valid) {
      ...
      this.carService.add(carModel).subscribe(
        (carAddSuccessResponse) => {
          if (this.carImagesFiles.length === 0) {
            //No pictures, just car added
            ...
            //If car pictures will be uploaded
            if (this.carImagesFiles.length > 5) {
              ...
              this.uploadAllImagesToServer(
                this.carImagesFiles,
                carAddSuccessResponse.data
              ).then((unUploadFileList) => {
                let unUploadedFiles: UploadFile[] = unUploadFileList;
                if (unUploadedFiles.length === 0) {
                  this.toastrService.success(
                    'Yeni araç ve resimleri başarıyla eklendi',
                    'İşlem başarılı'
                  );
                  this.closeCarAddModal();
                } else {
                  ...
                  this.toastrService.warning(
                    'Yeni araç başarıyla eklendi fakat bazı resimler yüklenemedi. Yüklenemeyen dosyalar: ' +
                      failFileNameMessage,
        ...
        (carAddFailResponse) => {
          ...
          if (
            carAddFailResponse.error.ValidationErrors &&
            carAddFailResponse.error.ValidationErrors.length > 0
          ) {
            for (
              let i = 0;
              i < carAddFailResponse.error.ValidationErrors.length;
              i++
            ) {
              this.toastrService.error(
                carAddFailResponse.error.ValidationErrors[i].ErrorMessage,
                'Doğrulama hatası'
              );
            }
          }
          //Back-end Validation ok but other errors
            ...
            this.toastrService.error(
              carAddFailResponse.error.Message,
              'Araç eklenemedi'
            );
          }
        }
      ...
      if (uploadFiles.length > 0) {
        ...
        const allUploads = new Promise<void>(async (resolveAllUploads) => {
          ...
          for (const file of uploadFiles) {
            await this.uploadImageToServer(file, carId)
              .then((fileStatus) => {
  ...
  private uploadImageToServer(
    ...
    return new Promise<UploadFile>((result) => {
      this.carImagesService.uploadImage(uploadFile.file, carId).subscribe(
        (uploadSuccess) => {
    ...
    if (imageList.length !== 0) {
      if (this.carImagesFiles.length < 5) {
        for (let i = 0; i < imageList.length; i++) {
          ...
          uploadFile.uploadStatus = false;
          let preselectedFile = this.carImagesFiles.find(
